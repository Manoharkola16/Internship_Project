...
// import { fetchBlogs, createBlog } from "../../../BlogSlice"
...
//     // pass author, title, content up to Home
//     onPost({ author: author || "You", title, content });
...
//   // read blogs from Redux
//   const { blogs = [], loading: blogsLoading, error: blogsError } = useSelector(
//     (state) => state.blog ?? { blogs: [], loading: false, error: null }
//   );
...
//   useEffect(() => {
//     // fetch blogs from backend via Redux
//     dispatch(fetchBlogs());
//   }, [dispatch]);
...
//   const handlePostBlog = async ({ author, title, content }) => {
//     try {
//       // createBlog thunk will add the blog to Redux on success
//       await dispatch(createBlog({ author, title, content })).unwrap();
//       setShowCreateModal(false);
//       setActiveNav("home");
//     } catch (err) {
...
//             {blogsLoading && <div className="text-sm text-gray-500">Loading...</div>}
//             {blogsError && <div className="text-sm text-red-500">Error: {String(blogsError)}</div>}

//             <div className="flex flex-col gap-4">
//               {(blogs && blogs.length > 0 ? blogs : []).map((post) => (
//                 <PostCard key={post._id ?? post.id} post={post} />
//               ))}
//               {/* if no blogs, you can show a helpful empty state */}
//               {(!blogs || blogs.length === 0) && !blogsLoading && (
//                 <div className="text-sm text-gray-500">No posts yet — create the first one!</div>
//               )}
//             </div>
//           </section>
//         </main>
